...
// 根据phone 更改用户信息
exports.ChangeUserSign = function(user_name, user_phone, user_password, user_age, user_city, user_signature, callback) {
    console.log(user_name+" " +user_phone+ " " +user_password, user_age, user_city+"ChangeNew=ChangeNew=CzhangeNew");
    var  updateSql = 'UPDATE user SET user_name=?, user_password=?, user_age=?, user_city=? , user_signature=? WHERE user_phone='+user_phone;
    var updateParams = [user_name, user_password, user_age, user_city, user_signature];
    pool.getConnection(function(err, connection) {
        connection.query(updateSql, updateParams, function(err,result) {
            if (err) { 
                throw err; 
